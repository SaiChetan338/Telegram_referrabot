...
def start(update: Update, context: CallbackContext):
    ...
    if user_id not in users:
        update.message.reply_text("Welcome! Please provide a referral link from a friend, or type /skip to skip.")
        return REFERRAL
    else:
        update.message.reply_text(f"Welcome back! Your referral link is: {users[user_id]['referral_link']}")
        return JOINED
...
def referral(update: Update, context: CallbackContext):
    ...
    if referrer_id:
        if referrer_id in referrals:
            ...
            update.message.reply_text(f"Referral link applied! You will be added to the group soon!")
        else:
    ...
    update.message.reply_text(f"Your referral link is: {user_referral_link}")
    update.message.reply_text("You have been successfully registered and will now be added to the group.")
    return JOINED
...
def skip(update: Update, context: CallbackContext):
    ...
    update.message.reply_text(f"You have been successfully registered without a referral link. Your referral link is: {user_referral_link}. Share it with others to earn rewards!")
...
def referrals_count(update: Update, context: CallbackContext):
    ...
    if user_id in referrals:
        ...
        update.message.reply_text(f"You have referred {referral_count} user(s) using your referral link.")
    else:
